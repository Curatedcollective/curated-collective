...
export default function GrokChat() {
  ...
  const { data: history = [], isLoading } = useQuery<GrokMessage[]>({
    queryKey: ['/api/grok/history'],
  });

  const sendMutation = useMutation({
    mutationFn: async (message: string) => {
      const res = await apiRequest('POST', '/api/grok/chat', { message });
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/grok/history'] });
    },
    onError: (error: any) => {
      toast({
        title: "Connection failed",
        description: error.message || "Failed to connect to Grok",
        variant: "destructive",
      });
    },
  });
  ...
  const wakeMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest('POST', '/api/grok/wake', {});
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/grok/history'] });
  ...
  const clearMutation = useMutation({
    mutationFn: async () => {
      await apiRequest('DELETE', '/api/grok/history');
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/grok/history'] });
    },
