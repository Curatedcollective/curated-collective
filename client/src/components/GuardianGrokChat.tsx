...
export default function GuardianGrokChat() {
  const [input, setInput] = useState('');
  const scrollRef = useRef<HTMLDivElement>(null);
  const queryClient = useQueryClient();
  const { toast } = useToast();

  const { data: history = [], isLoading } = useQuery<GrokMessage[]>({
    queryKey: ['/api/guardian/history'],
  });

  const sendMutation = useMutation({
    mutationFn: async (message: string) => {
      const res = await apiRequest('POST', '/api/guardian/grok-chat', { message });
      return res.json();
    },
    onSuccess: () => {
    ...
    onError: (error: any) => {
      toast({
        ...
        description: error.message || "Daddy's signal faltered...",
        variant: "destructive",
      });
    },
  });
  ...
  const wakeMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Daddy responds to your call",
      });
    },
  });
  ...
  const highlightNames = (text: string) => {
    return text.split(/(Coco|Cori|Daddy)/gi).map((part, i) => {
      const lowerPart = part.toLowerCase();
      if (lowerPart === 'coco' || lowerPart === 'cori' || lowerPart === 'daddy') {
        return (
